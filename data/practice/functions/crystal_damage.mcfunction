...
# keep the 100x mirror in step with the rollback above
execute if score diff health matches ..9 run scoreboard players operation #virtual_health100 zc_ctrl = current health
execute if score diff health matches ..9 run scoreboard players operation #virtual_health100 zc_ctrl *= #c100 zc_ctrl
