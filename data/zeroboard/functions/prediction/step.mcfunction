...
# real path any more. runs as the zc_dragon_sim_pos marker.
...
execute at @s if entity @e[type=minecraft:armor_stand,tag=zc_dragon_sim_target,limit=1,distance=..10] run function zeroboard:prediction/finish
# diverged - 150+ blocks out, not tracking the real path any more
execute at @s if entity @e[type=minecraft:armor_stand,tag=zc_dragon_sim_target,limit=1,distance=150..] run function zeroboard:prediction/finish
